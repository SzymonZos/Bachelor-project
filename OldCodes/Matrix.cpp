...
{
	this->rows = rows; //!< Zmiennej rows z klasy nadajê wartoœæ rows.
...
CMatrix::CMatrix(uint32_t rows, uint32_t columns, double **mat) //!< Konstruktor nadaj¹cy macierzy w klasie wartoœæ ró¿n¹ od zera.
{
	this->rows = rows;
...
{
	if (row < 0 || column < 0 || row >= rows || column >= columns) throw invalid_argument("Error"); //!< Jeœli podana liczba jest ujemna albo wychodzi poza zakres tablicy.
	return matrix[row][column];
}


double *CMatrix::operator[](uint32_t row) throw(invalid_argument) //!< Operator [] bêdzie zwraca³ konkretny wiersz macierzy.
{
